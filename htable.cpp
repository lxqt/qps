...
{ 
	...
	else p.drawPixmap(0,0,*pix_vcross);
}
...
//	if( !dragging) return;
	...
	if(e->buttons()==Qt::LeftButton  //  Button state 
		and htable->options & HTBL_REORDER_COLS) {
		{
			...
			if(!dragging and thold > 5) {
				...
				floatHead->setTitle(htable->title(click_col),cellWidth(click_col) ,height());
				floatHead->show();
				//vp->show();
			}
			...
			//	vp->setMovable(htable->columnMovable(htable->columnMovable(vcol) and htable->columnMovable(click_col) ));
			//	vp->move(vcol_offx - p->geometry().x()	,p->geometry().y() - 13);
				//vp->move(e->x()	,10);
				old_pos=vcol_offx;
			}			
	...
	if(col!=-1) // ToolTip 
	{	
		//	for(int i=0;i<1024*1024;i++)	htable->tipText(col); // memory leak test
		QString s = htable->tipText(col);
		
		if(!s.isEmpty()) QToolTip::showText(e->globalPos(), s);
	}
...
{	
	if(e->button()==Qt::LeftButton) //Returns the button that caused the event.
	{
		int col = findCol(e->x());
		if(col < 0)
		...
		if(dragging)
		{
			...
			if(click_col >= 0)
			{
				htable->moveCol(click_col, vcol); //call  Pstable::moveCol(int col, int place) 
				//should emit !!
...
bool TableBody::isCellChanged(int row,int col)
{
	...
	else
	{
		...
		if(		attr->text==str
				...
				and (attr->size.height()>=tmp_size.height())
		...
		//	if (row==29 and col==0)
		//	printf("(%d %d) xpos=%d %d sorted=%d %d sel=%d %d str=%s %s\n",attr->size.height(),tmp_size.height(),
...
{
	// *** sequence important !!!
//	if(isCellChanged(row,col)==false)	{	if(use_cache==true)	return; }

	CellAttribute *attr=tablecache.value(row-topCell(),col-leftCell()); //save cache
//	isCellChanged(row,col);
	...
	lineColor.setRgb(
			(baseColor.red()*0.5+altColor.red()*0.5), 
  ...
  //  const int gridHint = style()->styleHint(QStyle::SH_Table_GridLineColor, &opt, this);
	...
	if(col==0 and htable->treemode==true ) {
		...
		if( true /*lines*/) {
			...
			for(int level = d, prow = row; level >= 0 and prow >= 0;level--, prow = htable->parentRow(prow)) 
            {
				if(level==d) continue;
			...
			if(testFlag)
			{
				...
				if(!htable->lastChild(row)) opt.state = opt.state | QStyle::State_Sibling ; // | 
			}
		...
		if(true /*folding*/ ) {
			...
			if(fs != HeadedTable::Leaf) {
				...
				if(fs!=HeadedTable::Closed) opt.state = opt.state  | QStyle::State_Open;
			}
	...
	//p->drawText(gap, 0, w , h, Qt::TextIncludeTrailingSpace | Qt::IncludeTrailingSpaces | Qt::AlignVCenter | htable->alignment_col[col],htable->text(row,col)); 
	//p->drawText(gap, 0, w , h, Qt::TextIncludeTrailingSpace | Qt::AlignVCenter | htable->alignment_col[col],htable->text(row,col)); 
	p->drawText(gap, 0, w , h, Qt::AlignVCenter | htable->alignment_col[col],htable->text(row,col)); 
...
void TableBody::mousePressEvent(QMouseEvent *e)
{
	...
	{
		if(!htable->isSelected(row)) htable->selectOnlyOne(row);
		emit htable->selectionChanged();
...
// Bottle Neck ? no
void TableBody::leaveEvent ( QEvent * ) 
{
	emit htable->outOfCell();
}
...
void TableBody::mouseMoveEvent(QMouseEvent *e)
{
	...
	if(e->buttons() & Qt::ControlModifier || gadget_click)	return;
...
void HeadedTable::fontChange( const QFont & oldFont ) 
{
	...
	if (fontHeight%2!=0) fontHeight++;	// for pretty fold-lines
...
void TableHead::checkProfile()
{
	...
	if(maxViewCol<0) maxViewCol=numCols();
...
{	
    ...
    QRect viewR = viewRect();
	if(viewR.y()!=0) printf(" qps: ooooophss....\n");
	int maxViewRow = findRow(viewR.height());
	if(maxViewRow<0) maxViewRow=numRows();
	int maxViewCol = findCol(viewR.width());
	if(maxViewCol<0) maxViewCol=numCols();
	tablecache.setCol(maxViewCol);
