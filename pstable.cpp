...
{
	if(col!=0) return;
	if(procview->cats[col]->id != F_CMD) return;
	...
	if(n==1) return;
#ifdef LINUX 
	if(pi->pid!=pi->tgid) return; // LINUX 
...
Pstable::Pstable(QWidget *parent,Procview *pv)
	: HeadedTable(parent,0)
{
	procview=pv;
//	connect(this, SIGNAL(selectionChanged(const Svec<int> *)),SLOT(selection_update(const Svec<int> *))); //DEL?
//	connect(this, SIGNAL(selectionChanged()),SLOT(selection_update())); // when procinfo clicked
	connect(this, SIGNAL(titleClicked(int)), SLOT(setSortColumn(int)));
...
void Pstable::selection_update(const Svec<int> *rows)
{
	...
	for(int i = 0; i < rows->size(); i++) {
		int row = (*rows)[i];
		procview->linear_procs[row]->selected = isSelected(row); // WRONG !!!
	}
...
{
	//qDebug("Pstable::setTreeMode() %d , procview.treeview =%d\n",treemode,procview->treeview);
...
//	if(size!=numRows())
	{
	//	printf("Warnning: this is the bug.. wrong! %d %d\n",size,numRows());

	}
	if(procview->current_gen!=kgen)
	{
//		printf("Warnning: maybe this is the bug.. wrong! %d %d\n",kgen,procview->current_gen);
//		setAutoUpdate(false);  // ********** important for GUI-thread
