/****************************************************************************
** Meta object code from reading C++ file 'fieldsel.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.6)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../fieldsel.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'fieldsel.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.6. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_FieldSelect[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: signature, parameters, type, tag, flags
      13,   12,   12,   12, 0x05,
      30,   12,   12,   12, 0x05,

 // slots: signature, parameters, type, tag, flags
      49,   12,   12,   12, 0x0a,
      69,   12,   12,   12, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_FieldSelect[] = {
    "FieldSelect\0\0added_field(int)\0"
    "removed_field(int)\0field_toggled(bool)\0"
    "closed()\0"
};

void FieldSelect::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        FieldSelect *_t = static_cast<FieldSelect *>(_o);
        switch (_id) {
        case 0: _t->added_field((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->removed_field((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->field_toggled((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 3: _t->closed(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData FieldSelect::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject FieldSelect::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_FieldSelect,
      qt_meta_data_FieldSelect, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &FieldSelect::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *FieldSelect::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *FieldSelect::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_FieldSelect))
        return static_cast<void*>(const_cast< FieldSelect*>(this));
    return QDialog::qt_metacast(_clname);
}

int FieldSelect::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void FieldSelect::added_field(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void FieldSelect::removed_field(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
