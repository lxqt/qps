	...
	*/
	unsigned int guest_utime,cguest_utime;
		...
		mini_sscanf(sbuf, "%d (%S) %c %d %d %d %d %d" 
			...
			&nthreads /* number of threads v2.6 */,
			/* itrealvalue */
	...
	*/
	...
	if((buf= read_proc_file2(path,"io")) !=0 ) 
	{
		...
		//if(io_read_prev!=0)
		{
			...
			// NOTE: Kbps right????
			io_read_KBps =(io_read-io_read_prev)/proc->update_msec; // not accurate....
	...
	if( (len = readlink(path, buf, sizeof(buf) - 1)) > 0) {
				...
				|| sscanf(buf, "socket:[%lu]", &ino) > 0) // Linux 2.1 upper
		{
			...
			if(si) {
				...
				sprintf(buf, "%s socket %lu",si->proto == Sockinfo::TCP ? "TCP" : "UDP", ino);
	...
	while(fgets(buf, sizeof(buf), f)) {
		...
		sscanf(buf, "%x: %x %x %x %x %x %ld %n",
			// Num	refcount protocol flags type state indoe path
				&q, &q, &q /*protocol*/ , &us.flags, &type, &state, &us.inode, &n);
		us.name = buf + n;
		us.type = type;
		us.state = state;
...
bool Procinfo::read_environ()
{
	...
	{
		char buf[128];
		sprintf(buf,"Kernel Bug= wrong environments !  please, check /proc/%d/environ !",pid);
		size=strlen(buf)+1;
		if(file_size>size)
			strcpy(envblock,buf);
	}
...
Proc::Proc()
{
	...
	categories.insert(F_TPGID,new Cat_int("TPGID", "Process group ID of tty owner", 6, &Procinfo::tpgid));

	categories.insert(F_USER, new Cat_string("USER", "Owner (*=suid root, +=suid a user)",&Procinfo::username));
	categories.insert(F_GROUP,new Cat_string("GROUP", "Group name (*=sgid other)",&Procinfo::groupname));
	...
	categories.insert(F_SUID, new Cat_int("SUID", "Saved user ID (Posix)", 6,&Procinfo::suid));
	categories.insert(F_FSUID,new Cat_int("FSUID", "File system user ID", 6,&Procinfo::fsuid));
	...
	categories.insert(F_SGID, new Cat_int("SGID", "Saved group ID (Posix)", 6,&Procinfo::sgid));
	categories.insert(F_FSGID,new Cat_int("FSGID", "File system group ID", 6,&Procinfo::fsgid));
	...
	categories.insert(F_NLWP, new Cat_int("NLWP", "Number of tasks(threads) in task group",5, &Procinfo::nthreads));
	...
	categories.insert(F_MEM,  new Cat_memory("MEM", "memory usage (RSS-SHARE)", 8, &Procinfo::mem));
	categories.insert(F_TRS,  new Cat_memory("TRS", "Text(code) resident set size", 8, &Procinfo::trs));
	categories.insert(F_DRS,  new Cat_memory("DRS", "Data resident set size(malloc+global variable)", 8, &Procinfo::drs));
	categories.insert(F_STACK,new Cat_memory("STACK", "Stack size", 8, &Procinfo::stack));
	categories.insert(F_SHARE,new Cat_memory("SHARE", "Shared memory with other libs", 8, &Procinfo::share));
	categories.insert(F_SWAP, new Cat_swap  ("SWAP", "Kbytes on swap device"));
	categories.insert(F_IOR	,new Cat_memory("IO_R", "io read (file)", 8, &Procinfo::io_read));
	categories.insert(F_IOW	,new Cat_memory("IO_W", "io write (file)", 8, &Procinfo::io_write));

	categories.insert(F_DT,   new Cat_uintl("DT", "Number of dirty (non-written) pages", 7, &Procinfo::dt));
	...
	categories.insert(F_PMEM,  new Cat_percent("%MEM", "Percentage of memory used (RSS/total mem)", 6, &Procinfo::pmem));
	...
	categories.insert(F_CPUNUM,new Cat_int("CPU", "CPU the process is executing on (SMP system)", 3, &Procinfo::which_cpu));

	categories.insert(F_CMD, new Cat_string("Process Name", "the process name",	&Procinfo::command));
//	categories.insert(F_PROCESSNAME, new Cat_string("Process Name", "the process name",	&Procinfo::command));
