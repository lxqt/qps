...
//COMMON
Cat_percent::Cat_percent(const char *heading, const char *explain, int w, float Procinfo::*member)
...
// table view - sort
void Procview::linearize_tree(QVector<Procinfo *> *ps, int level, int prow, bool hide)
{
...
void Procview::set_fields_list(int fields[])
{
	...
	{
		if(fields[i]==F_CPUNUM and Proc::num_cpus<2) continue; // not correctly work
...
///	Proc::read_sockets(); // test
	...
	// remove non-existing processes, remove Procinfos of nonexisting processes
	QHash<int,Procinfo*>::iterator i;
	for ( i = procs.begin(); i != procs.end();)
	{
		Procinfo *p = i.value();
