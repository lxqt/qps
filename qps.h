...
#ifdef SOLARIS
		static bool normalize_nice;		// whether nice should be normalized (normal nice is 0 etc)
...
//		static bool comm_is_magic;	//DEL, auto-remove COMM when switching
...
#endif
			COLOR_CPU_IDLE,
			COLOR_MEM_USED, COLOR_MEM_BUFF, COLOR_MEM_CACHE, COLOR_MEM_FREE,
			COLOR_SWAP_USED, COLOR_SWAP_FREE, COLOR_SWAP_WARN,
			COLOR_LOAD_BG, COLOR_LOAD_FG, COLOR_LOAD_LINES,
			COLOR_SELECTION_BG, COLOR_SELECTION_FG,
			NUM_COLORS
		};
		...
		enum menuid {
			...
			MENU_RENICE, MENU_SCHED,
			MENU_DETAILS,
			MENU_PARENT, MENU_CHILD, MENU_DYNASTY,
			MENU_SIGTERM, MENU_SIGHUP, MENU_SIGINT, MENU_SIGKILL, MENU_OTHERS,
			MENU_PROCS, MENU_FIELDS, MENU_CUSTOM,
			MENU_PATH, MENU_INFOBAR, MENU_CTRLBAR, MENU_CUMUL, MENU_PREFS,
			MENU_ADD_HIDDEN,MENU_REMOVE_HIDDEN, MENU_STATUS,
#ifdef MOSIX
			POPUP_MIGRATE,
#endif
